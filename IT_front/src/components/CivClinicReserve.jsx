...
import { dateTime } from "./schemas/schema";



const CivClinicReserve = () => {
    const componentRef = useRef();
    let { GetCivClinincs, allCivClinics, GetSpecificClininc,spesClinic  } = useContext(authcontext);
    ...
    useEffect(() => {
        if (spesClinic.clinicName && count.data) {
            handlePrint(count.data.count, spesClinic.clinicName, spesClinic.doctorName, cilvil, hour_min, mydate)
        }
    }, [count]);

    const addCount = async (clinicId) => {
        
        
        const voice = "مريض رقم "
    ...
    return (
        <>
            <div className="mx-5  p-5 mt-5">
                <h5 className="fs-1">مدنيين</h5>



                                {allCivClinics.length > 0 ?
                                    <div className="content d-flex flex-column flex-wrap ">
                                        {allCivClinics.map(clinic => (
                                            <div key={clinic.id}>
                                            <span  className="">
                                                { <button className="btn bg-green fs-2 bolder mt-5 " onClick={() => addCount(clinic.id)} >عيادة {clinic.clinicName}</button> }
                                                {/* { <button className="btn bg-green fs-2 bolder " onClick={() => addCount(clinic.id)} data-bs-toggle="modal" data-bs-target="#exampleModal" data-bs-whatever="@fat" >عيادة {clinic.clinicName}</button> } */}
                                            ...
                                            <div className="modal-dialog">
                                                ...
                                                <h2>عيادة : <span>الأسنان</span> </h2>
                                                        ...
                                                        <div className="modal-footer d-flex justify-content-center">
                                                            ...
                                                            <button type="submit" className=" btn2 w-30 fs-4 mt-3 " data-bs-dismiss="modal" onClick={()=>handlePrint(count.count, spesClinic.clinicName, spesClinic.doctorName, cilvil, hour_min, mydate)}>
                                                                طباعه
                                                            </button>
                                                        </div>
                                                    </div>
...
export default CivClinicReserve;
